  ...
  <div class="form-wrap">
    ...
    <div class="toggle-wrap">
      <div class="toggle-inner">
        <!-- logo -->
        <div class="logo-row" @tap="$router.push('/')">
        ...
        <div class="form-panel">
          ...
          <div class="form-row suffix">
            ...
            <svg-icon v-if="isShowRePassword" @tap="isShowRePassword=false" iconClass="eye-open" className="suffix-icon"></svg-icon>
            <svg-icon v-else @tap="isShowRePassword=true" iconClass="eye" className="suffix-icon"></svg-icon>
          </div>
          <div class="form-row suffix">
            <label>{{$t('code')}}</label>
            <input type="text" :placeholder="$t('placeholder.code')" autocomplete="off" v-model="dataForm.code">
            <svg-icon iconClass="code" className="suffix-icon" @tap="isShowCode=true"></svg-icon>
          </div>
          <XButton type="primary" class="b-but" @tap.native="submitData">{{$t('register')}}</XButton>
          <div class="link-row">
    ...
    <XDialog :show.sync="isShowCode" :hide-on-blur="true">
      ...
      <div @tap="isShowCode=false"><span class="vux-close"></span></div>
    </XDialog>
